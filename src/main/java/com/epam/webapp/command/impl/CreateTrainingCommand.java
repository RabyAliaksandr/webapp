...
public class CreateTrainingCommand implements Command {
  ...
  public String execute(HttpServletRequest request) throws CommandException {
    ...
    int mentorId = Integer.parseInt(request.getParameter(CommandConst.MENTOR_ID));
    String trainingDescription = request.getParameter(CommandConst.DESCRIPTION);
    boolean done;
    try {
    ...
    if (done) {
      ...
      return ConfigurationManager.getProperty(CommandConst.TRAININGS_PAGE);
    }
    request.getSession().setAttribute(CommandConst.MESSAGE_ABOUT_CHANGES,
